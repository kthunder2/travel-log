...
void deleteTravel (struct tas *aa, int del)
{
    ...
    //doesn't work properly! first node should be deleted with this if statement but it only frees the memory of ID number, not whole node.
    //idk why.
    ...
    //if the node is not the first one, it can delete it without problem. weird.
